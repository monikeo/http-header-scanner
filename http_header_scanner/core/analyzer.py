...
from http_header_scanner.models.headers import SECURITY_HEADERS, HEADER_CATEGORIES
...
class SecurityAnalyzer:
    ...
    def _analyze_csp(self, policy: str) -> ContentSecurityPolicyAnalysis:
        ...
        directives = [d.strip() for d in policy.splite(";") if d.strip()]
        ...
        for directive, value in analysis["directives"].items():
        ...
        if analysis["unsafe_directives"]:
            analysis["recommendations"].append("Remove 'unsafe-inline' and 'unsafe-eval' from CSP directive")

        if analysis["missing_directives"]:
            analysis["recommendations"].append(f"Add missing critical directives: {', '.join(analysis['missing_directives'])}")
            return analysis
    ...
    def _analyze_tls(self, url: str) -> TLSFinding:
        ...
        try:
            ...
            return {
                    ...
                    "cipher_strength": cipher[0] if cipher else "Unkown",
                    "vulnerabilities":
                    self._get_tls_vulnerabilities(tls_version)
            }
